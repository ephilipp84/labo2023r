...
dtrain[, clase_binaria_1 := as.numeric(clase_ternaria == "BAJA+2")]

dtrain <- select(dtrain, -clase_ternaria)

dapply[, clase_binaria_1 := clase_ternaria]

dapply <- select(dapply, -clase_ternaria)

dtrain2 <- dtrain

dtrain <- dtrain2[sample(1:164682,164682,replace=T),] 

# Establezco cuales son los campos que puedo usar para la prediccion
# el copy() es por la Lazy Evaluation
campos_buenos <- copy(setdiff(colnames(dtrain), c("clase_binaria_1")))
...
for (arbolito in 1:PARAM$num_trees_max) {
  ...
  formulita <- paste0("clase_binaria_1 ~ ", campos_random)
  ...
  if (arbolito %in% grabar) {
    ...
    umbral_corte <- ((1/40 + 1/40 * 1064/1268) / 2) * arbolito
